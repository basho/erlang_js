...
typedef struct _jaegermonkey_drv_t {
  ErlDrvPort port;
  jaegermonkey_vm *vm;
...
typedef struct _js_call_t {
  jaegermonkey_drv_t *driver_data;
  ErlDrvBinary *args;
} js_call;
...
static ErlDrvEntry jaegermonkey_drv_entry = {
    init,                             /* init */
    start,                            /* startup */
    stop,                             /* shutdown */
    NULL,                             /* output */
...
void send_ok_response(jaegermonkey_drv_t *dd, const char *call_id) {
...
void send_error_string_response(jaegermonkey_drv_t *dd, const char *call_id, const char *msg) {
  ErlDrvTermData terms[] = {ERL_DRV_BUF2BINARY, (ErlDrvTermData) call_id, strlen(call_id),
...
void send_string_response(jaegermonkey_drv_t *dd, const char *call_id, const char *result) {
  ErlDrvTermData terms[] = {ERL_DRV_BUF2BINARY, (ErlDrvTermData) call_id, strlen(call_id),
                            ERL_DRV_ATOM, dd->atom_ok,
...
void unknown_command(jaegermonkey_drv_t *dd, const char *call_id) {
  ErlDrvTermData terms[] = {ERL_DRV_BUF2BINARY, (ErlDrvTermData) call_id, strlen(call_id),
                            ERL_DRV_ATOM, dd->atom_error,
...
void run_js(void *jsargs) {
  ...
  jaegermonkey_drv_t *dd = call_data->driver_data;
...
DRIVER_INIT(jaegermonkey_drv) {
  return &jaegermonkey_drv_entry;
}
...
static ErlDrvData start(ErlDrvPort port, char *cmd) {
  jaegermonkey_drv_t *retval = (jaegermonkey_drv_t*) driver_alloc(sizeof(jaegermonkey_drv_t));
  retval->port = port;
  retval->shutdown = 0;
...
static void stop(ErlDrvData handle) {
  jaegermonkey_drv_t *dd = (jaegermonkey_drv_t*) handle;
  sm_stop(dd->vm);
  if (dd->shutdown) {
    sm_shutdown();
  }
